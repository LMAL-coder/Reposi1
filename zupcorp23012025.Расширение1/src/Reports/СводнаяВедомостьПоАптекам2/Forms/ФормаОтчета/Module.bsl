...
Функция СформироватьОтчетНаСервере()
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	ДифференциацияБонусаПоСетиСрезПоследних.НормаНочь30Дней КАК НормаНочь30Дней,
	             |	ДифференциацияБонусаПоСетиСрезПоследних.НормаНочь31Дней КАК НормаНочь31Дней,
	             |	ДифференциацияБонусаПоСетиСрезПоследних.Бонус1Оптика КАК Бонус3
	             |ИЗ
	             |	РегистрСведений.ДифференциацияБонусаПоСети.СрезПоследних(&ДатаПо, ) КАК ДифференциацияБонусаПоСетиСрезПоследних";
	РезультатПоказатели=Запрос.Выполнить().Выгрузить();
	...
	Запрос.Текст = 
		...
		|	ДанныеПоАптекам.БонусЗаОптику КАК БонусЗаОптику
		|ИЗ
		|	РегистрСведений.ДанныеПоАптекам КАК ДанныеПоАптекам
	...
	//Общие итоги по аптекам
			...
			ПремияМин1=0;
			ПремияМакс1=0;
			ПремияЗаКатегориюСтоимость1=0;
			ДоплатаЗаКатегорию1=0;
			ПремияНочнаяСтоимость1=0;
			ПремияНочнаяСумма1=0;
	...
	Пока ВыборкаАптеки.Следующий() Цикл
		...
		//Для итоги по аптеке
			...
			ПремияМин=0;
			ПремияМакс=0;
			ПремияЗаКатегориюСтоимость=0;
			ДоплатаЗаКатегорию=0;
			ПремияНочнаяСтоимость=0;
			ПремияНочнаяСумма=0;
			
			//ОсновноеМестоРаботы="Да";
			//ТабельныйНомер="";
		
		Сотр=ВыборкаАптеки.Выбрать(); //Обходим по сотрудникам данной аптеки
		Пока Сотр.Следующий() Цикл
			...
			Если НЕ Рез3.Пустой() Тогда
				...
				Пока Показатели.Следующий() Цикл
					...
					СтоимостьПраздники=Показатели.СтоимостьПраздники;
					СтоимостьДень=Показатели.СтоимостьДень;
					СтоимостьПраздникиНочь=Показатели.СтоимостьПраздникиНочь;
					ПремияНочная=Показатели.ПремияНочная;
        	...
        	КонецЕсли; 
			  //день
			//НачисленияОтработаноДень=Начисления[0].Оклад0_166_Часы;
			//k=0;
			//Если НачисленияОтработаноДень>0 Тогда
			//	k=Сотр.ВремяДень/НачисленияОтработаноДень;
			//КонецЕсли;
			
			//Обл2.Параметры.Часы166=Сотр.ВремяДень;
			//Обл2.Параметры.Сумма166=ОКР(Начисления[0].Оклад0_166Сумма*k,2);
			ВремяДень=Сотр.ВремяДень-Сотр.ПраздникиВремяДень;
			Обл2.Параметры.Часы166=ВремяДень;
			Обл2.Параметры.Сумма166=ОКР(ВремяДень*СтоимостьДень*Коэффициент,2);
			
			Обл2.Параметры.ПремияЗаКатегориюСтоимость=ДоплатаЗаКатегорию;
			Обл2.Параметры.ДоплатаЗаКатегорию=ОКР(ВремяДень*ДоплатаЗаКатегорию*Коэффициент,2);
			
			//ночь
			ВремяНочь= Сотр.ВремяНочь-Сотр.ПраздникиВремяНочь;
			Обл2.Параметры.ЧасыНочь=ВремяНочь;
			Обл2.Параметры.СуммаНочь=ОКР(ВремяНочь*СтоимостьНочногоЧаса*Коэффициент,2);
			
			Обл2.Параметры.ПремияНочнаяСтоимость=ПремияНочная;
			Обл2.Параметры.ПремияНочнаяСумма=ОКР(ВремяНочь*ПремияНочная*Коэффициент,2);
			//Обл2.Параметры.ЧасыНочь=Сотр.ВремяНочь;
			//НачисленияОтработаноНочь=Начисления[0].Ночь_Часы;
			//k=0;
			//Если НачисленияОтработаноНочь>0 Тогда
			//	k=Сотр.ВремяНочь/НачисленияОтработаноНочь;
			//КонецЕсли;
			//Обл2.Параметры.СуммаНочь=ОКР(Начисления[0].Ночь_Сумма*k,2);
			
			//Учитываем праздники
			Обл2.Параметры.ПраздникиДень=Сотр.ПраздникиВремяДень;
			Обл2.Параметры.ПраздникиНочь=Сотр.ПраздникиВремяНочь;
			Обл2.Параметры.ПраздникиДеньСумма=ОКР(Сотр.ПраздникиВремяДень*СтоимостьПраздники*Коэффициент,2);
			Обл2.Параметры.ПраздникиНочьСумма=ОКР(Сотр.ПраздникиВремяНочь*СтоимостьПраздникиНочь*Коэффициент,2);
			...
			Обл2.Параметры.СуммаВсего=Обл2.Параметры.Сумма166+Обл2.Параметры.СуммаНочь+Обл2.Параметры.ПраздникиДеньСумма+Обл2.Параметры.ПраздникиНочьСумма
			+Обл2.Параметры.ПремияНочнаяСумма+Обл2.Параметры.ДоплатаЗаКатегорию;
			...
			//КонецЕсли;
			//Обл2.Параметры.Бонус1=ОКР(Начисления[0].БонусНеДТМ*k1,2);
			
			//ДТМ
			Обл2.Параметры.ЛичнаяВыручка2=Сотр.ЛичнаяВыручкаДТМ;
			Обл2.Параметры.Бонус2=ОКР(Сотр.БонусПоДМ*Коэффициент,2);
			
			//Оптика    !!!!! добавить в макет
			Обл2.Параметры.ЛичнаяВыручкаОптика=Сотр.ЛичнаяВыручкаОптика;
			Обл2.Параметры.БонусОптика=ОКР(Сотр.БонусЗаОптику*Коэффициент,2);
			
			Обл2.Параметры.ИтогоБонус12=Обл2.Параметры.Бонус1+Обл2.Параметры.Бонус2+Обл2.Параметры.БонусОптика;
			...
			Если Начисления[0].ОтработаноВсего>0 Тогда
				 k1=Обл2.Параметры.ОтработаноВсего/Начисления[0].ОтработаноВсего;
			КонецЕсли;
			Обл2.Параметры.БонусМ=ОКР(Начисления[0].БонусАкции*k1,2);
			Обл2.Параметры.БонусЗМЗ=ОКР(Начисления[0].БонусЗМЗ*k1,2);
			...
			// !!!!! добавить в макет
			Обл2.Параметры.ДопВыплаты2=ОКР(Начисления[0].Доплата2*k1,2);
			...
			Обл2.Параметры.ИтогоДоГраниц=Обл2.Параметры.СуммаВсего+ Обл2.Параметры.ИтогоБонус12+Обл2.Параметры.БонусМ
										+Обл2.Параметры.БонусЗМЗ;
			...
			ЧасыДеньПраздники=ЧасыДеньПраздники+Обл2.Параметры.ПраздникиДень;
			СуммаПраздникиДень=СуммаПраздникиДень+Обл2.Параметры.ПраздникиДеньСумма;
			ЧасыНочьПраздники=ЧасыНочьПраздники+Обл2.Параметры.ПраздникиНочь;
			СуммаПраздникиНочь=СуммаПраздникиНочь+Обл2.Параметры.ПраздникиНочьСумма;
			...
			ВыручкаОптика=ВыручкаОптика+Обл2.Параметры.ЛичнаяВыручкаОптика;
			БонусЗаОптику=БонусЗаОптику+Обл2.Параметры.БонусОптика;	
			...
			Доплата2=Доплата2+Обл2.Параметры.ДопВыплаты2;
			ПремияМин=ПремияМин+Обл2.Параметры.ПремияМин;
			ПремияМакс=ПремияМакс+Обл2.Параметры.ПремияМакс;
			
			ПремияЗаКатегориюСтоимость=ПремияЗаКатегориюСтоимость+Обл2.Параметры.ПремияЗаКатегориюСтоимость;
			ДоплатаЗаКатегорию=ДоплатаЗаКатегорию+Обл2.Параметры.ДоплатаЗаКатегорию;
			ПремияНочнаяСтоимость=ПремияНочнаяСтоимость+Обл2.Параметры.ПремияНочнаяСтоимость;
			ПремияНочнаяСумма=ПремияНочнаяСумма+Обл2.Параметры.ПремияНочнаяСумма;
		КонецЦикла;
		Обл3=Макет.ПолучитьОбласть("Итоги");
		...
		//!!!!!
		Обл3.Параметры.ИтогоПраздникиДень=ЧасыДеньПраздники; 
		Обл3.Параметры.ИтогоПраздникиДеньСумма=СуммаПраздникиДень;
		Обл3.Параметры.ИтогоПраздникиНочь=ЧасыНочьПраздники; 
		Обл3.Параметры.ИтогоПраздникиНочьСумма=СуммаПраздникиНочь;
		...
		//!!!!!
		Обл3.Параметры.ИтогоВыручкаОптика=ВыручкаОптика;
		Обл3.Параметры.ИтогоБонусЗаОптику=БонусЗаОптику;
		...
		//!!!!!
		Обл3.Параметры.ИтогоДопВыплаты2=Доплата2;
		
		Обл3.Параметры.ИтогоПремияМин=ПремияМин;
		Обл3.Параметры.ИтогоПремияМакс=ПремияМакс;
		
		Обл3.Параметры.ИтогПремияНочнаяСтоимость=ПремияНочнаяСтоимость;
		Обл3.Параметры.ИтогоПремияНочнаяСумма=ПремияНочнаяСумма;
		Обл3.Параметры.ИтогоПремияЗаКатегориюСтоимость=ПремияЗаКатегориюСтоимость;
		Обл3.Параметры.ИтогоДоплатаЗаКатегорию=ДоплатаЗаКатегорию;
		...
		ВыручкаОптика1=ВыручкаОптика1+ВыручкаОптика;
		БонусЗаОптику1=БонусЗаОптику1+БонусЗаОптику;
		...
		Доплата21=Доплата21+Доплата2;	
		ПремияМин1=ПремияМин1+ПремияМин;
		ПремияМакс1=ПремияМакс1+ПремияМакс;
		
		ПремияЗаКатегориюСтоимость1=ПремияЗаКатегориюСтоимость1+ПремияЗаКатегориюСтоимость;
		ДоплатаЗаКатегорию1=ДоплатаЗаКатегорию1+ДоплатаЗаКатегорию;
		ПремияНочнаяСтоимость1=ПремияНочнаяСтоимость1+ПремияНочнаяСтоимость;
		ПремияНочнаяСумма1=ПремияНочнаяСумма1+ПремияНочнаяСумма;
	КонецЦикла;
	...
	Обл3.Параметры.ИтогоСуммаНочь=СуммаНочь1;
	    //!!!!!!!!!!
		Обл3.Параметры.ИтогоПраздникиДень=ЧасыДеньПраздники1; 
		Обл3.Параметры.ИтогоПраздникиДеньСумма=СуммаПраздникиДень1;
		Обл3.Параметры.ИтогоПраздникиНочь=ЧасыНочьПраздники1; 
		Обл3.Параметры.ИтогоПраздникиНочьСумма=СуммаПраздникиНочь1;
	...
	Обл3.Параметры.ИтогоБонус2=БонусДТМ1;
		...
		//!!!!!!!
		Обл3.Параметры.ИтогоВыручкаОптика=ВыручкаОптика1;
		Обл3.Параметры.ИтогоБонусЗаОптику=БонусЗаОптику1;
	...
	Обл3.Параметры.ИтогоДопВыплаты1=Доплата11;
		//!!!!!!
		Обл3.Параметры.ИтогоДопВыплаты2=Доплата21;
	Обл3.Параметры.ИтогоПремияМин=ПремияМин1;
	Обл3.Параметры.ИтогоПремияМакс=ПремияМакс1;
	
	Обл3.Параметры.ИтогПремияНочнаяСтоимость=ПремияНочнаяСтоимость1;
	Обл3.Параметры.ИтогоПремияНочнаяСумма=ПремияНочнаяСумма1;
	Обл3.Параметры.ИтогоПремияЗаКатегориюСтоимость=ПремияЗаКатегориюСтоимость1;
	Обл3.Параметры.ИтогоДоплатаЗаКатегорию=ДоплатаЗаКатегорию1;
