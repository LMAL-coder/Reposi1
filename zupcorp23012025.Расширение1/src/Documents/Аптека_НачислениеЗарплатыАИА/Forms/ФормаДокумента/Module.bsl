...
Функция ПолучитьСотрудниковПоОсновному()
	...
	Запрос.УстановитьПараметр("ПодразделениеРаботы",справочники.ПодразделенияОрганизаций.НайтиПоНаименованию("Интернет-аптека",истина));	
	...
	|	И ДанныеПоАптекам.ПодразделениеРаботы = &ПодразделениеРаботы";
	Если ЗначениеЗаполнено(Объект.Подразделение)тогда
		запрос.Текст=запрос.Текст+"
...
Процедура РассчитатьДокумент() 
	...
	Для каждого НоваяСтрока из Объект.ТЧАптека  цикл 
		...
		Попытка
			...
			//Если Консультант
			Если НоваяСтрока.Должность=Справочники.Должности.НайтиПоНаименованию("Консультант",Истина) Тогда 
				НоваяСтрока.ИтогоБонусов=(МасФЛ[0].БонусЗаЛичнуюВыручку+МасФЛ[0].БонусПоДМ)*Коэффициент;
				НоваяСтрока.БонусНеДТМ=МасФЛ[0].БонусЗаЛичнуюВыручку*Коэффициент;
				НоваяСтрока.БонусДТМ=МасФЛ[0].БонусПоДМ*Коэффициент;
				НоваяСтрока.БонусЗвонки=МасФЛ[0].БонусЗвонки*Коэффициент;
				НоваяСтрока.БонусСтрокиЗаказов=МасФЛ[0].БонусСтрокиЗаказов*Коэффициент;
				//находим пределы  
				...
				НоваяСтрока.ИтогоДоГраниц=НоваяСтрока.ИтогоЗаОтработанноеВремя+НоваяСтрока.ИтогоБонусов //+НоваяСтрока.БонусАкции+НоваяСтрока.БонусЗМЗ
				+НоваяСтрока.БонусОценка+НоваяСтрока.Доплата2+НоваяСтрока.ДоплатаЗаКатегорию+НоваяСтрока.БонусЗвонки+НоваяСтрока.БонусСтрокиЗаказов;
				...
				КонецЕсли
			Иначе //Для разбощика товаров и курьеров
				 НоваяСтрока.Отработановсего=МасФЛ[0].ВремяДень+МасФЛ[0].ВремяНочь;
				НоваяСтрока.Ночь_Часы=МасФЛ[0].ВремяНочь;
				НоваяСтрока.Ночь_Сумма=МасФЛ[0].ВремяНочь*СтоимостьНочногоЧаса*Коэффициент;
				ВремяДень=МасФЛ[0].ВремяДень;
			...
			КонецЕсли //Для разбощика товаров и курьеров
